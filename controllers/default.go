...
func (c *MainController) ErrorPage(){
	...
	switch code {
	case "401": c.Abort(code)
	case "403": c.Abort(code)
	case "404": c.Abort(code)
	case "500": c.Abort(code)
	case "503": c.Abort(code)
	case "db": c.Abort(code)
	case "custom": c.Abort(code)
	case "unknow": c.Abort(code)
	default:
